...
#globtherm = read.csv("Data/Traits/GlobalTherm_upload_10_11_17.csv", header = TRUE, na.strings = "")
ectotherms = subset(globtherm, Class == "Lepidosauria", select = c(Genus, Species, Class, Tmax, REF_max, tmin)) # Subsetting for Lizards (Lepidosuria) only
...
require(taxize)
synonyms <- readRDS(file = "synonyms.rds") #Already looked them up so just uploading previous results
